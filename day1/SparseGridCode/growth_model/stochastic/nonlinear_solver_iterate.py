...
def iterate(k_init, paramL, grid_list):
    n_agents = paramL['n_agents']
    c_bar = paramL['c_bar']
    c_up = paramL['c_up']
    l_bar = paramL['l_bar']
    l_up = paramL['l_up']
    inv_bar = paramL['inv_bar']
    inv_up = paramL['inv_up']
    ...
    def eval_f(X):
        return EV_F_ITER(X, k_init, paramL, grid_list)
        
    def eval_grad_f(X):
        return EV_GRAD_F_ITER(X, k_init, paramL, grid_list)
        
    def eval_g(X):
        return EV_G_ITER(X, k_init, paramL)
        
    def eval_jac_g(X, flag):
        return EV_JAC_G_ITER(X, flag, k_init, paramL)
    ...
    #for num in range(to_print.shape[0]):
    #    f.write(str(to_print[num])+"\t")
    #f.write("\n")
    #f.close()
