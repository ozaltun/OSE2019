...
def initial(k_init, paramL):
    ...
    grid_pt_box=k_init
    X_L[:n_agents]=paramL['c_bar']
    X_U[:n_agents]=paramL['c_up']

    X_L[n_agents:2*n_agents]=paramL['l_bar']
    X_U[n_agents:2*n_agents]=paramL['l_up']

    X_L[2*n_agents:3*n_agents]=paramL['inv_bar']
    X_U[2*n_agents:3*n_agents]=paramL['inv_up']

    # Set bounds for the constraints 
    G_L[:n_agents]=paramL['c_bar']
    G_U[:n_agents]=paramL['c_up']

    G_L[n_agents:2*n_agents]=paramL['l_bar']
    G_U[n_agents:2*n_agents]=paramL['l_up']

    G_L[2*n_agents:3*n_agents]=paramL['inv_bar']
    G_U[2*n_agents:3*n_agents]=paramL['inv_up']
    ...
    def eval_f(X):
        return EV_F(X, k_init, paramL)
    
    def eval_grad_f(X):
        return EV_GRAD_F(X,k_init, paramL)
    
    def eval_g(X):
        return EV_G(X, k_init, paramL)
        
    def eval_jac_g(X, flag):
        return EV_JAC_G(X, flag, k_init, paramL)
